...
import { page, card, button, colors, Badge, Progress, Masthead, Countdown, Slider, useLocal, euro } from '../components/RichDemoUI';
...
export default function Mercato(){
  ...
  return <main style={page('intermediaire')}>
    ...
    <p style={{color:'#C9D4E4',fontSize:19,maxWidth:900}}>Chaque été, les Guests sans affiliation et les contrats arrivant à échéance entrent en Mercato. Médias et Guests se positionnent mutuellement, sur 3 paliers de contrat (2K / 4K / 6K), indexés au score TSAAK.</p>

    <section style={{...card(colors.intermediaire),display:'flex',flexWrap:'wrap',gap:24,alignItems:'center',justifyContent:'space-between'}}>
    ...
    <h2 style={{marginTop:30}}>Guests en Mercato</h2>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:16}}>
      {GUESTS.map(g=>{
        const signed = st.contracts[g.id];
        return <div key={g.id} onClick={()=>setSt({...st,selected:g.id})} style={{...card(g.id===guest.id?colors.intermediaire:'rgba(255,255,255,.12)'),cursor:'pointer',padding:0,overflow:'hidden'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1.2fr .8fr',gap:22,marginTop:26}}>
      <section style={card(colors.intermediaire)}>
        ...
        {contract ? <div style={{padding:16,borderRadius:14,background:colors.core+'14',border:`1px solid ${colors.core}`}}>
          ...
          <div style={{display:'grid',gap:12}}>
            {offers.map(o=><div key={o.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 16px',borderRadius:14,background:'rgba(255,255,255,.05)',border:`1px solid ${o.recommended?colors.talent:'rgba(255,255,255,.14)'}`}}>
      ...
      <section style={card(colors.core)}>
        <h2>Investissement Kaastbase</h2>
